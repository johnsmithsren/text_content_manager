...
#     with session_scope() as db_session:
#         support_columns = ("key", "title", "content", "source_from",
#                            "source_url", "status")
...
#         count = query.count()
#         results = query.order_by(News.create_time.desc()).slice(start,
#                                                                 end).all()
...
#                            "icon"
#                            "source_url", "publish_time", "to_static", "status")
...
#         support_columns = ("key", "title", "content", "source_from", "summary",
#                            "icon"
#                            "source_url", "publish_time", "status")
#         for column in support_columns:
#             if column in data:
...
#             query = query.filter(text(_filter))
#         count = query.count()
#         results = query.order_by(Advertisement.publish_time.desc()).slice(
#             start, end).all()
...
#         if result is None:
#             return None
#         support_columns = ("key", "title", "content", "image", "color",
#                            "back_color", "publish_time", "url", "status")
#         for column in support_columns:
#             if column in data:
#                 setattr(result, column, data[column])
#         result.publish_time = now
...
def test_list():
    now = time.time()
    list1 = {i: 2 * i for i in range(1000000)}
    list2 = {i: 2 * i for i in range(10000000)}
    print "@@@@@@__", now
    for item in range(1000000):
        list1.get(item)
    print "@@@@@@__", time.time() - now
    for item in range(1000000):
        list2.get(item)
    print "@@@@@@__", time.time() - now
